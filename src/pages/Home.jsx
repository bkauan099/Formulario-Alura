...
function Home() {
  ...
  const handleSubmit = (e) => {
    ...
    const camposObrigatorios = [
      'nome', 'data', 'cpf', 'sexo', 'email', 'tel', 'rg', 'residencia',
  ...
  return (
    ...
    <div className='bg-white mt-15 mx-auto flex flex-col w-150 rounded-lg '>
      <header className='mx-10 mt-5 ' >
        ...
        <div>
          <h1 className='text-[24px] mb-2 font-'>Formulário de Inscrição</h1>
          <p className='text-[16px]'>Preencha os dados abaixo para fazer sua inscrição no Programa Trilhas</p>
        </div>
      </header>

      <form className='mx-10 mt-10 max-w-[600px]' onSubmit={ handleSubmit }>
        <div class="formulario-dados">
          <h2 className='text-[16px]'>Informações do Participante</h2>
          <div className='mt-5 mb-5'>
            <label className='text-[14px]'>Nome Completo</label>
            <input type='text' name="nome" class="campo" placeholder='Ex: João da Silva Pereira' onChange={handleChange} />
            {formErros.nome && <p className="campo__erro">{formErros.nome}</p>}
          </div>
          <div className='mt-5 mb-5'>
            <label className='text-[14px]'>Data de Nascimento</label>
            <input type='date' name='data' class="campo" onChange={handleChange} />
            {formErros.data && <p className="campo__erro">{formErros.data}</p>}
          </div>
          <div className='mt-5 mb-5'>
            <label className='text-[14px]'>CPF</label>
            <input class="campo" type="text" name="cpf" placeholder="000.000.000-00" pattern="\d{3}\.\d{3}\.\d{3}-\d{2}" onChange={handleChange}  />
            {formErros.cpf && <p className="campo__erro">{formErros.cpf}</p>}
          </div>
          <div>
            <label className='text-[14px]'>Sexo</label>
            <select name="sexo" id="sexo" class="campo" defaultValue="" onChange={handleChange} >
          ...
          <div className='mt-5 mb-5'>
            ...
            <input type='email' name='email' class="campo" placeholder="Seu@email.com" onChange={handleChange}  />
            {formErros.email && <p className="campo__erro">{formErros.email}</p>}
          </div>
          <div className='mt-5 mb-5'>
            <label className='text-[14px]'>Telefone</label>
            <input type='tel' name='tel' class="campo" placeholder="(00) 00000-0000" pattern="\([0-9]{2}\) [0-9]{5}-[0-9]{4}" onChange={handleChange}  />
            {formErros.tel && <p className="campo__erro">{formErros.tel}</p>}
          </div>
          <div>
            <p className='mb-1 text-[14px]'>Documento de Identidade (PDF)</p>
        ...
        <div className="Formulario-endereco mt-10">
          <h2 className='text-[16px]'>Endereço Residencial</h2>
          <div className='mt-5 mb-5'>
            <label className='text-[14px]'>CEP</label>
            <input type='text' name='cep' class="campo" placeholder="00000-000" pattern="\d{5}-\d{3}" onChange={handleChange}  />
            {formErros.cep && <p className="campo__erro">{formErros.cep}</p>}
          </div>
          <div className='mt-5 mb-5 flex gap-4'>
            <div className='w-[100%]'>
              <label className='text-[14px]'>Rua</label>
            ...
            <div>
              <label className='text-[14px]'>Número</label>
              <input type='text' name='numero' class="campo" onChange={handleChange}  />
          ...
          <div className='mt-5 mb-5 flex gap-4'>
            <div className='w-[100%]'>
              <label className='text-[14px]'>Cidade</label>
            ...
            <div>
              <label className='text-[14px]'>Estado</label>
          ...
          <div>
            <p className='mb-1 text-[14px]'>Comprovante de Residência (PDF)</p>
        ...
        <div className="Formulario-trilhas mt-10">
          <h2 className="mb-4 text-[16px]">Trilhas de Aprendizagem</h2>
          <p className="text-[14px] mb-4">Selecione apenas uma trilha</p>
